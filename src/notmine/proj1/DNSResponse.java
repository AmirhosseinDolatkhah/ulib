...
public class DNSResponse {
    ...
    private DNSRequest.QueryType queryType;
    private boolean noRecords = false;

	public DNSResponse(byte[] response, int requestSize, DNSRequest.QueryType queryType) {
    ...
    private DNSRequest.QueryType getQTYPEFromByteArray(byte[] qType) {
        if (qType[0] == 0) {
            if (qType[1] == 1) {
                return DNSRequest.QueryType.A;
            } else if (qType[1] == 2) {
                return DNSRequest.QueryType.NS;
            } else if (qType[1] == 15) {
                return  DNSRequest.QueryType.MX;
            } else if (qType[1] == 5) {
            	return DNSRequest.QueryType.CNAME;
            }else {
            	return DNSRequest.QueryType.OTHER;
