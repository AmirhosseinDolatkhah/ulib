...
public class DnsResponse{
    ...
    private int NSCount;
    private int ARCount;
    private final DNSRecord[] answerRecords;
    private final DNSRecord[] additionalRecords;
    ...
    private void parseHeader(){
        ...
        //RA
        boolean RA = getBit(response[3], 7) == 1;
    ...
    private DNSRecord parseAnswer(int index){
        ...
        switch (result.getQueryType()) {
            case A:
                ...
                break;
            case NS:
                result.setDomain(parseNSTypeRDATA(countByte));
                break;
            case MX:
                result.setDomain(parseMXTypeRDATA(countByte, result));
                break;
            case CNAME:
                result.setDomain(parseCNAMETypeRDATA(countByte));
                break;
            case OTHER:
            	break;
        }
    ...
    private String parseNSTypeRDATA(int countByte) {
		rDataEntry result = getDomainFromIndex(countByte);

        return result.getDomain();
    }

    private String parseMXTypeRDATA(int countByte, DNSRecord record) {
    ...
    private String parseCNAMETypeRDATA(int countByte) {
		rDataEntry result = getDomainFromIndex(countByte);
    ...
    private rDataEntry getDomainFromIndex(int index){
    	...
    	result.setDomain(domain.toString());
